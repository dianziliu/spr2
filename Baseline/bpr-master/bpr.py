...
class BPR(object):
    ...
    def init(self,data):
        ...
        self.user_factors = np.random.random_sample((self.num_users,self.D))/10
        self.item_factors = np.random.random_sample((self.num_items,self.D))/10
...
class Sampler(object):
    ...
    def sample_negative_item(self,user_items):
        j = self.random_item()
        tmp=user_items.toarray().reshape(-1)
        while tmp[j]:
            j = self.random_item()
        return j
...
if __name__ == '__main__':
    ...
    # 
    train_path="data/MLLSratings.csv"
    # df=df[["userId","movieId","rating"]]
    # data = mmread("Baseline/data/bprMLLS.mtx").tocsr()
    data=np.zeros((800,10000))
